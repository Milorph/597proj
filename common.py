# shared helpers for both groups: load real csvs, sample, aggregate flows, metrics
# no other local deps -- this file + the two folders is the whole repo
...
def sample_dataset(pop, seed=None, n_benign=200_000, attack_min=4_000, attack_max=6_200):
    ...
    # spread roughly evenly across the 5 types (+/-15% jitter)
    raw = (total / 5) * rng.uniform(0.85, 1.15, 5)
...
def per_attack_detection_rate(y_multi, pred):
    ...
    for atk in ATTACK_TYPES:
        mask = (y_multi == atk)
        n = int(mask.sum())
        out[atk] = {"detected": int(pred[mask].sum()) if n else 0, "total": n,
                    "detection_rate": (int(pred[mask].sum()) / n) if n else float("nan")}
    return out
